...
def search_venues():
    ...
    get_searched_venues = Venue.query.filter(Venue.name.ilike(f'%{search_venue}%')).all()
    count = len(get_searched_venues)
    data = []
    for s_venue in get_searched_venues:
        data.append(s_venue)
...
def shows():
    ...
    #data = Venue.query.join(Artist, Venue.artists).all()
